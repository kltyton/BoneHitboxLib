...
Minecraft 26.1.2 entity persistence uses `ValueInput` / `ValueOutput`. Vanilla data components are primarily the `ItemStack` data model and are not used as this cross-loader entity store. / Minecraft 26.1.2 的实体持久化使用 `ValueInput` / `ValueOutput`。原版数据组件主要是 `ItemStack` 数据模型，本库不把它作为跨 loader 的实体存储。
