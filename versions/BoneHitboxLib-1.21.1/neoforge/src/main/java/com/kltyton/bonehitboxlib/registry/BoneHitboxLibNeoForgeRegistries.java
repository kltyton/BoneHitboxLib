...
public final class BoneHitboxLibNeoForgeRegistries {
    ...
    public static final DeferredHolder<EntityType<?>, EntityType<ObbZombieTestEntity>> OBB_ZOMBIE_TEST = ENTITY_TYPES.register("obb_zombie_test", () -> EntityType.Builder.of(ObbZombieTestEntity::new, MobCategory.MONSTER).sized(0.6F, 1.95F).clientTrackingRange(10).updateInterval(3).build(Constants.id("obb_zombie_test").toString()));

    public static final DeferredHolder<EntityType<?>, EntityType<SoftObbRavagerTestEntity>> SOFT_OBB_RAVAGER_TEST = ENTITY_TYPES.register("soft_obb_ravager_test", () -> EntityType.Builder.of(SoftObbRavagerTestEntity::new, MobCategory.MONSTER).sized(1.95F, 2.2F).clientTrackingRange(10).updateInterval(3).build(Constants.id("soft_obb_ravager_test").toString()));

    public static final DeferredHolder<EntityType<?>, EntityType<HardObbRavagerTestEntity>> HARD_OBB_RAVAGER_TEST = ENTITY_TYPES.register("hard_obb_ravager_test", () -> EntityType.Builder.of(HardObbRavagerTestEntity::new, MobCategory.MONSTER).sized(1.95F, 2.2F).clientTrackingRange(10).updateInterval(3).build(Constants.id("hard_obb_ravager_test").toString()));
