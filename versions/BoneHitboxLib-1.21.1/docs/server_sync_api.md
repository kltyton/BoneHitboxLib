...
Minecraft 1.21.1 entity persistence uses `CompoundTag` and `NbtOps`. Vanilla data components are primarily the `ItemStack` data model and are not used as this cross-loader entity store. / Minecraft 1.21.1 的实体持久化使用 `CompoundTag` 和 `NbtOps`。原版数据组件主要是 `ItemStack` 数据模型，本库不把它作为跨 loader 的实体存储。
