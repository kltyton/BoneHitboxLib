...
`held_item:left_item` and `held_item:right_item` use actual `ItemInHandLayer` transforms. Each baked model produces one enclosing OBB; special items contribute their rendered model cubes. Flat geometry receives a small nonzero thickness. Empty hands produce no item OBB. / 左右手持物使用实际手臂和物品 display 变换；每个烘焙模型生成一个包围 OBB，特殊物品捕获其模型 cube。平面几何保留极薄厚度，空手不生成物品框。
