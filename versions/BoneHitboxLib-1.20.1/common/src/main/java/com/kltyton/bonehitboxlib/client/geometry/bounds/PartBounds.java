...
public record PartBounds(AABB renderBounds, AABB localBounds, VoxelShape localShape, Matrix4f localToRender, Matrix4f renderToLocal) {
    ...
    public static PartBounds fromVanillaCube(Matrix4f pose, ModelPart.Cube cube) {
        ...
        cube.compile(new com.mojang.blaze3d.vertex.PoseStack().last(),
                new com.kltyton.bonehitboxlib.client.render.item.DiscardingVertexConsumer() {
                    @Override
                    public com.mojang.blaze3d.vertex.VertexConsumer vertex(double x, double y, double z) {
                        builder.include((float) x, (float) y, (float) z);
                        return this;
                    }
                }, 0, 0, 1, 1, 1, 1);
    ...
    public static final class BoundsBuilder {
        ...
        private BoundsBuilder(Matrix4f pose) {
            this.pose = com.kltyton.bonehitboxlib.client.render.WorldRenderPose.cameraRelative(pose);
        }
