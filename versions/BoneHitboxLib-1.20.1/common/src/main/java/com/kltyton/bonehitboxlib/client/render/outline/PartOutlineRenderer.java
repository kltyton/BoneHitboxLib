...
public final class PartOutlineRenderer {
        ...
        try {
            ...
            for (Outline outline : PENDING) {
                ...
                outline.bounds().localShape().forAllEdges((x1, y1, z1, x2, y2, z2) -> {
                    ...
                    lines.vertex(pose.pose(), (float)x1, (float)y1, (float)z1).color(SELECTED_COLOR).normal(pose.normal(), normal.x(), normal.y(), normal.z()).endVertex();
                    lines.vertex(pose.pose(), (float)x2, (float)y2, (float)z2).color(SELECTED_COLOR).normal(pose.normal(), normal.x(), normal.y(), normal.z()).endVertex();
                });
            }
    ...
    private static void vertex(com.mojang.blaze3d.vertex.VertexConsumer buffer, PoseStack.Pose pose, double x, double y, double z) {
        buffer.vertex(pose.pose(), (float) x, (float) y, (float) z).color(DAMAGE_FLASH_COLOR).endVertex();
    }
}
