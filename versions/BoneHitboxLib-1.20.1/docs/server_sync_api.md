...
Minecraft 1.20.1 entity persistence uses `CompoundTag` and `NbtOps`. Vanilla data components are primarily the `ItemStack` data model and are not used as this cross-loader entity store. / Minecraft 1.20.1 的实体持久化使用 `CompoundTag` 和 `NbtOps`。原版数据组件主要是 `ItemStack` 数据模型，本库不把它作为跨 loader 的实体存储。
...
BoneHitboxLib advances every client-tracked `GeoBoneHitboxEntity` once per game tick through GeckoLib 4 `GeoModel.handleAnimations` with partial tick 1.0. This runs the normal controller and marker timeline outside the camera frustum without drawing. / BoneHitboxLib 每个客户端游戏 tick 都会通过 GeckoLib 4 的 `GeoModel.handleAnimations`，以 partial tick 1.0 推进已追踪实体的控制器和 marker 时间轴。实体在视锥外仍可推进，不提交绘制。
...
GeckoLib imports are isolated in optional compatibility packages, including the marker bridge, controller snapshot extractor and logical ticker. A guarded facade invokes them only while GeckoLib is present. All packet, server, context, registration and geometry APIs remain loadable without GeckoLib. / GeckoLib 导入隔离在可选兼容包中，包括 marker 桥、控制器快照和逻辑 ticker；统一门面仅在 GeckoLib 存在时调用。网络包、服务端、上下文、注册和几何 API 不依赖 GeckoLib。
...
Entity snapshot payloads now use `bonehitboxlib:entity_parts_v3`; Forge transport uses version `3`. Both sides must run a matching build. Existing entity save keys remain unchanged. Server snapshot, contact, selection and skill tables clear on server shutdown. / 实体快照包升级为 `bonehitboxlib:entity_parts_v3`，Forge 传输版本为 `3`，两端需使用匹配构建；实体存档键不变。服务端关闭时清除快照、接触、选择和技能运行表。
