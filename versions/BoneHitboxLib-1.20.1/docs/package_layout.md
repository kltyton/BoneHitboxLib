...
The package migration changes Java imports. Registry and persisted-data IDs remain unchanged. The current controller-list extension uses entity snapshot payload `bonehitboxlib:entity_parts_v3` and Forge transport version `3`; server and clients need matching builds. / 包迁移会改变 Java import；注册 ID 和持久化数据 ID 不变。本次多控制器扩展使用 `bonehitboxlib:entity_parts_v3` 和 Forge 传输版本 `3`，两端需使用匹配构建。
