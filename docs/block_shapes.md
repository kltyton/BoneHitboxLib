...
适用 Minecraft 26.2、Fabric/NeoForge。形状读取已打包的 Java 方块模型 `elements`，沿用 Eden Realm 的旋转、凸多面体、按单元格裁切、四方向旋转、联动放置/拆除和交互转发逻辑。运行时不依赖 GeckoLib 或客户端渲染。
...
这里的 `generator`、`includeClient`、`output` 与 `automaticBlocks` 由接入方已有 datagen 入口提供。生成器输出四朝向、仅原点可见的 multipart blockstate；存在方块物品时输出 26.2 的 `assets/<namespace>/items/<item>.json`。原始模型、纹理、语言和掉落表仍由方块所属 Mod 提供。没有引入另一套跨 Loader 注册器。
...
客户端资源包、运行时特殊渲染器和 Gecko 动画不改变这份服务端物理几何。动态或特殊方块、第三方形状覆盖、配置热重载和多人同步效果须人工验收。切换名单与两个开关后重新进入存档，是人工验收的基本检查步骤。

## 人工验收清单

按项目规则，下面均由用户执行；构建和进存档启动检查不能代替这些验收。

1. 在两个 Loader 上分别检查默认值、强制模式、空白名单、精确 ID、命名空间、黑白名单冲突及两个开关关闭后的行为；检查原版和模组方块、无模型方块、完整方块及原本无碰撞的装饰物。
2. 用同一个倾斜且跨格模型分别注册 AABB、OBB 两种模式，四方向放置，检查轮廓、碰撞和占位一致。
3. 分别破坏原点与链接格；测试右键、拾取方块、爆炸、掉落数量、重进存档和跨区块边缘放置。
4. 检查占位被方块或实体阻挡、世界边界、高度边界、区块未加载时能否正确拒绝放置。
5. 玩家左右手换物、主手切换、空手、平面物品及不同 display 变换，检查第一/第三人称世界持物 OBB。
6. Gecko 技能检查单点、窗口、多控制器、暂停/停止/切换/循环、视锥外推进、移除与换维度；多人重复上报不应重复开窗。
7. 检查实体 HARD/SOFT/NONE、攻击时临时 HURT、接触结束恢复、移动承载、掉线重连与服务器重启。
8. 联机检查恶意或延迟报告、服务端技能许可钩子；客户端辅助姿态不能作为完整反作弊依据。高速旋转、瞬移穿透及远坐标浮点精度也需单独验收。


## 0.2.1 公共元素解析入口

`ModelShapeCache.buildForElements(JsonArray, Direction)` 返回公开 `ShapeSet`，供下游离线几何检查或已获得模型元素的调用方复用同一算法。`Cell`、`ShapeSet.localShape(Cell)` 同时公开；`localShapes` 与 `occupiedCells` 在结果构造时复制为不可修改集合。已有按资源 ID 的缓存和多格基类行为不变。Eden Realm 当前 26.2 NeoForge 消费工程使用该 API，并内置本库 0.2.1 jar。
