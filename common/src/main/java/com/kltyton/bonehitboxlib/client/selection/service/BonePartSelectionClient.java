...
public final class BonePartSelectionClient {
    ...
    public static void finishLocalPlayerMovement(Entity entity, Vec3 movement) {
        ...
        synchronized (LOCK) {
            ...
            player.setOnGroundWithMovement(true, player.horizontalCollision, movement);
    ...
    public static boolean shouldSuppressFullEntityRed(EntityRenderState state) {
        if (!shouldReplaceFullRed(state)) {
            return false;
        }
        return shouldSuppressFullEntityRed(VisualPartId.ownerKey(state));
    }
    ...
    private static SupportCandidate applyLocalSupport(Player player, Minecraft minecraft, SupportCandidate support,
        ...
        player.verticalCollision = true;
        player.verticalCollisionBelow = true;
        player.setOnGroundWithMovement(true, player.horizontalCollision, carryingMovement);
        player.resetFallDistance();
        activeSupport = support.anchor();
        return support;
    }
    ...
    private static boolean shouldReplaceFullRed(EntityRenderState state) {
        String ownerKey = VisualPartId.ownerKey(state);
        if (!ownerKey.startsWith("entity:")) {
            return false;
        }

        try {
            return shouldReplaceFullRed(entityById(Integer.parseInt(ownerKey.substring("entity:".length()))));
